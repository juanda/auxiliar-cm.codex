# Tema 16. Windows 10 - Esquema de repaso

## Versión oficial

La convocatoria 2026 fija `Windows 10` para las preguntas sobre Windows.

## Núcleo del tema

| Concepto | Definición breve |
| --- | --- |
| Explorador de archivos | Herramienta para navegar por archivos, carpetas y unidades. |
| `Windows + E` | Abre el Explorador. |
| Barra de direcciones | Muestra la ruta actual. |
| Este equipo | Unidades, dispositivos y carpetas principales. |
| Acceso rápido | Carpetas frecuentes/ancladas y archivos recientes. |
| Panel de vista previa | Permite ver ciertos archivos sin abrirlos. |
| Extensión | Caracteres tras el punto final del nombre. |
| Carpeta personal | Carpeta propia de cada usuario. |

## Preguntas historicas clave

| Id | Idea |
| --- | --- |
| `2023-E2-003` | Carpeta personal de usuario. |
| `2023-E2-011` | Quitar USB con seguridad evita pérdida de datos. |
| `2023-E2-017` | Número entre paréntesis al agrupar = archivos del grupo. |
| `2023-E2-019` | Extensión: caracteres tras punto final. |
| `2023-E2-025` | Compartir carpeta = acceso para otros usuarios de red. |
| `2023-E2-033` | Sin asociación: `¿Cómo quieres abrir este archivo?`. |
| `2025-E2-001` | `Windows + E` abre Explorador. |
| `2025-E2-002` | Administrador de tareas muestra procesos activos. |
| `2025-E2-003` | Espacio libre desde propiedades de unidad en Este equipo. |
| `2025-E2-004` | Ruta completa en barra de direcciones. |
| `2025-E2-005` | Panel de vista previa. |
| `2025-E2-031` | `diskmgmt.msc` abre Administrador de discos. |

## Comandos y herramientas

| Comando/herramienta | Uso |
| --- | --- |
| `diskmgmt.msc` | Administrador de discos. |
| `devmgmt.msc` | Administrador de dispositivos. |
| `services.msc` | Servicios. |
| `taskschd.msc` | Programador de tareas. |
| Administrador de tareas | Procesos y rendimiento. |
| Visor de eventos | Registros del sistema. |
| Sensor de almacenamiento | Liberar espacio. |

## Búsqueda

| Patrón | Uso |
| --- | --- |
| `*.pdf` | Archivos PDF. |
| `informe*` | Nombres que empiezan por informe. |
| `foto?.jpg` | Un carácter variable. |
| `*` | Cualquier cadena. |
| `?` | Un único carácter. |

## Trampas habituales

| Trampa | Respuesta correcta |
| --- | --- |
| Acceso rápido vs ubicación real | Acceso rápido contiene accesos. |
| Desanclar vs borrar | Desanclar no borra la carpeta. |
| Este equipo vs Acceso rápido | Este equipo muestra unidades/dispositivos. |
| Vista previa vs abrir | Vista previa no abre con la aplicación principal. |
| Compartir vs comprimir | Compartir da acceso; comprimir reduce/agrupa. |
| Ordenar vs agrupar | Ordenar reordena; agrupar separa por categorías. |
| `diskmgmt.msc` vs `devmgmt.msc` | Discos frente a dispositivos. |

## Repaso de una pagina

Prioridad alta:

- `Windows + E` abre el Explorador.
- Ruta completa: barra de direcciones.
- Contenido sin abrir: panel de vista previa.
- Espacio libre: `Este equipo` -> unidad -> `Propiedades`.
- Extensión: caracteres tras el punto final.
- Carpeta personal: perfil de cada usuario.
- Compartir carpeta: acceso a otros usuarios de red.
- USB: quitar de forma segura para evitar pérdida de datos.
- Agrupación: número entre paréntesis = archivos del grupo.
- Procesos activos: Administrador de tareas.
- Administrador de discos: `diskmgmt.msc`.
