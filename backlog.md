...
| 2026-08-23 | Investigación en opofimatica.com (demo pública con preguntas reales completas de Windows 10/11, Word y Excel; opopildora resultó ser una plataforma de pago sin preguntas visibles) para ampliar temas 16, 17 y 18 con una segunda tanda de contenidos y preguntas no cubiertos por la ronda anterior. Tema 16: comandos de red básicos (`ping`, `ipconfig`, `netstat`, DHCP), Windows Hello, Cortana, `regedit`, `certmgr.msc`, gestión de pantallas múltiples, accesos de Configuración y trampa del "Modo Dios" (no es función oficial); test de 65 a 80 preguntas (distribución 20/20/20/20). Tema 17: nota al pie (`Alt+Ctrl+O`), Dibujar tabla, autoajuste de tablas, Números de línea, ubicación de Opciones de impresión, Contar palabras, Editor/Traducir/Leer en voz alta, estilos personales, Tesauro, selección de frase y modo de compatibilidad; test de 65 a 80 preguntas (distribución 22/20/19/19). Tema 18: `Proteger libro` (Estructura) frente a `Proteger hoja`, opciones reales del formato condicional, vista Diseño de página, `Reducir hasta ajustar`, ejemplos trabajados de `BUSCARV`/`EXTRAE`/`CONCAT`/`SI`/`O`, funciones inexistentes como distractor y atajos adicionales; test de 75 a 90 preguntas (distribución 24/23/22/21). | Hecho |
