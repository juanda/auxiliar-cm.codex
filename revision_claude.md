...
- Temas Bloque I ya balanceados y sin tocar: 1, 3, 4, 6, 8, 11. Bloque II ya balanceados: 16, 17, 18. `test_bloque_I.md`, `test_bloque_II.md` (fuera de la P.24) y el resto de tests psicotécnicos no mostraban sesgo severo y se dejaron igual.
- **Segundo caso de sesgo severo encontrado en el tema 19 (Access/Power BI)**, detectado al leer el archivo completo para generar sus flashcards: el test embebido en `05_temario/tema_19_access_powerbi.md` tenía 33 de 35 respuestas correctas marcadas como "A" (94%). No se detectó en la revisión inicial porque, a diferencia de los demás temas de Bloque II, este es el único caso en que el test embebido **no** es idéntico a `07_tests/test_tema_19.md` (que ya estaba bien distribuido: 2/14/11/8). Corregido solo en `05_temario/tema_19_access_powerbi.md`, con la misma técnica de redistribución verificada (permutación exacta de las mismas 4 opciones, nueva distribución 9/9/9/8). `07_tests/test_tema_19.md` no se ha tocado porque ya estaba bien.
...
No se ha abordado en esta pasada: eliminar la duplicación literal entre test embebido y `07_tests/` (Hallazgo 3), reforzar con más preguntas predictivas de dificultad media-alta en Excel/Power BI, corregir en origen la clasificación de `2023-E2-006`, rellenar `00_convocatoria/programa_oficial.md`, introducir práctica cronometrada por tema, ni revisar las 15 preguntas psicotécnicas visuales/tabulares pendientes — quedan como trabajo pendiente descrito más abajo.
...
3. ~~**Crear flashcards de Bloque II** (16-21)~~ — **Hecho en esta rama** (337 tarjetas nuevas en 6 mazos).
4. **Reforzar Excel** (tablas dinámicas, gráficos) **y Power BI** con preguntas predictivas de dificultad media-alta, dado que son los puntos de menor densidad histórica y mayor tendencia de crecimiento.
5. **Eliminar la duplicación literal** entre el test embebido de cada tema y `07_tests/test_tema_XX.md` (confirmada en temas 6 y 20; probable en más), sustituyendo por preguntas realmente nuevas.
...
7. Introducir **práctica cronometrada por tema** en Bloque II, no solo en los simulacros completos, para entrenar el ritmo real de ~1 minuto por pregunta.
8. Corregir en origen la clasificación de `2023-E2-006` (es de Access, no de Outlook) en `02_preguntas_extraidas`/`03_clasificacion`.
9. Rellenar `00_convocatoria/programa_oficial.md` (volcando el contenido de `AGENTS.md`) o eliminar la fila "Hecho" del README/backlog mientras esté vacío.
10. Revisar manualmente las 15 preguntas psicotécnicas visuales/tabulares de 2025 pendientes (tarea 17 del backlog) — no es Bloque II pero es el bloque de mayor peso del examen.
